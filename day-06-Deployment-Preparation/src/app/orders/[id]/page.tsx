...
import { MyImage, order,} from "@/types";
...
export default async function OrderPage({ params }: params) {
  ...
  const res: order = await client.fetch(query);
  // console.log(res)
  const startDate = new Date(res._createdAt);
  const daystoAdd = 2;
      ...
      <section className="py-24 relative">
        <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
          <h2 className="font-manrope font-bold text-3xl sm:text-4xl leading-10 text-black mb-11">
            { res.paid ? "Your Order Confirmed" : "Your Order canceled"}
          </h2>
          <h6 className="font-medium text-xl leading-8 text-black mb-3">
            Hello, {res.customerName}
          </h6>
          <p className="font-normal text-lg leading-8 text-gray-500 mb-11">
            { res.paid ? "Your order has been completed and be delivery in only two days." :"Your order has been canceled. We apologize for the inconvenience." }
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-8 py-6 border-y border-gray-100 mb-6">
            <div className="box group">
              <p className="font-normal text-base leading-7 text-gray-500 mb-3 transition-all duration-500 group-hover:text-gray-700">
               {res.paid ? "Delivery Date" : "Order Date" }
              </p>
              <h6 className="font-semibold font-manrope text-2xl leading-9 text-black">
                {res.paid ? date :  res._createdAt?.slice(0,10)}
              </h6>
            </div>
            <div className="box group">
              <p className="font-normal text-base leading-7 text-gray-500 mb-3 transition-all duration-500 group-hover:text-gray-700">
              ...
              <h6 className="font-semibold line-clamp-1 font-manrope text-2xl leading-9 text-black">
                #{res._id}...
              </h6>
            </div>
            <div className="box group">
